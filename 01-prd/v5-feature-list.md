...
> **创建日期**：2025-12-31
> **最后更新**：2026-03-10（v1.5 同步）

---

## 功能点拆解来源

基于已实现的 Vue3 + TypeScript 可交互原型，按照以下页面结构拆解功能点：
...
| E3 | 场景过滤逻辑 | P0 | E1, B3 | `ResourceList.vue` | ✅ |
...
**卡片类型映射（原型 componentMap 实际注册）**：

| 资源类型 | 卡片组件 | 场景适用 | 备注 |
|----------|----------|----------|------|
| `video` | KnowledgeCard | preview, review | 核心卡片，含分段进度条 |
| `practice` | PracticeCard | extra | 难度标签 + 彩色进度段 |
| `guide` | GuideCard | review | 学案，含页数 |
| `read_text` | ReadTextCard | preview (仅文科) | 读课文 |
| `summary_note` | SummaryNoteCard | — | ⚠️ v1.3 起不渲染 |
| `premium_hook` | PremiumHookCard | extra | 培优课付费钩子 |
